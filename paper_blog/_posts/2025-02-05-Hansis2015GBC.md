...
- quantify LULCC fluxes and attribute them to land use activities and countries
...
- LULCC transition overlain by [map of potential natural vegetation](https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2007GB003153)
  ...
  - to allocate cropland and pasture within a grid cell, we proportionally reduce all existing natural plant functional types
- each plant functional type associated with two types of parameters:
  - C stock densities for different land types
  - response curves after transitions: [original curves](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.2307/1942531) were piecewise linear, we approximate them exponentially for efficient summation of different histories

### 2.3 Simulations
- eight different accounting schemes (#1 through #8) based on three model runs and different post processing approaches

#### 2.3.1 Model Runs
- instant C stock changes to new equilibrium: #6, #7, #8
- legacy scheme:
...
- transient results in vegetation much lower (at beginning of accounting period) than equilibrium values, in soil comparable
- lower stocks lead to lower emissions from clearing

### 4.3 Temporal Attribution of Emissions
- committed-flux approach much better constrained by actual observational data
...
- computationally efficient estimation of LULCC fluxes
- accounts for gross transitions (important for sub-grid scale LULCC), includes wood harvest, has temporal accounting layer
- resulting net emissions at high end of previous studies, but within published range
...
- In Fig. 2, why do the yellow dots not coincide with the end of the solid yellow curves? Or do they in Fig. 2b) but not in 2c) and 2d)?
...
- *However, the current generation of **dynamic global vegetation models** does not allow for attributing the resulting carbon fluxes to an individual LULCC event because of computational constraints.* **BLUE does!**
